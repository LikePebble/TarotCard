...
import { enFocusParagraphOf, enPositionParagraphOf } from "@/data/en-reading";
...
function reversedParagraphs(card: Card, locale: Locale): string[] {
  const text = locale === "en" ? reversedCards[card.slug]?.en : reversedCards[card.slug]?.ko;
  const fallback = locale === "en"
    ? "A reversed card can show an energy that is delayed, internalized, or overextended. Consider the upright meaning through that lens."
    : REVERSED_FALLBACK;
  return (text && text.length > 0 ? text : fallback).split("\n\n");
}

function nameOf(card: Card, locale: Locale): string {
  return locale === "en" ? card.nameEn : koCards[card.slug]?.nameKo ?? card.nameEn;
}

function descriptionOf(card: Card, locale: Locale): string[] {
  if (locale === "en") return card.en.description.split("\n\n");
 ...
 */
function UprightDetails({ children, locale }: { children: ReactNode; locale: Locale }) {
  return (
    <details className="group mt-5 border-t border-line pt-3 lg:max-w-[520px]">
      <summary className="flex min-h-11 cursor-pointer list-none items-center gap-1.5 text-[12.5px] text-muted hover:text-cream lg:text-[13.5px]">
        {locale === "en" ? "View upright meaning" : "정방향 해석 보기"}
        <CaretDown
          size={13}
          weight="bold"
...
function ReversedBadge({ locale }: { locale: Locale }) {
  return (
...
export function OneCardResult({
  ...
  const paragraphs = descriptionOf(card, locale);
  const themeParagraph = english
    ? enFocusParagraphOf(focus, card.slug)
    : focusParagraphOf(focus, card.slug, locale);
  ...
  const reversedTheme = reversed
    ? english ? enFocusParagraphOf(focus, card.slug, true) : reversedFocusParagraphOf(focus, card.slug)
    : null;
    ...
    >
      ...
      <div>
        ...
        <h1 className="mt-1 text-center font-display text-[30px] font-semibold lg:text-left lg:text-[44px]">
          {nameOf(card, locale)}
          {!english ? <span className="ml-1 text-base font-normal text-muted lg:text-[22px]">{card.nameEn}</span> : null}
          {reversed ? <ReversedBadge locale={locale} /> : null}
        </h1>
        ...
        <div className="mt-4 lg:mt-6">
          {reversed ? (
            <>
              <div className="space-y-3 font-serif text-[15px] text-body lg:max-w-[520px] lg:text-base">
                {reversedParagraphs(card, locale).map((p) => (
              ...
              <UprightDetails locale={locale}>{uprightContent}</UprightDetails>
            </>
          ) : (
            uprightContent
          )}
...
function ThemeBlock({
  ...
  return (
    <div className="mt-3.5 border-t border-line pt-3">
      <p className="text-[12.5px] text-gold lg:text-[13.5px]">
        {english
          ? `${focusLabelOf(focus, locale)} · ${positionKey === "past" ? "what this card has shaped" : positionKey === "present" ? "what this card invites you to notice" : "what this card may help you prepare for"}`
          : THEME_FRAMES[positionKey](focusLabelOf(focus, locale))}
      </p>
...
export function ThreeCardResult({
  ...
  const locale = useLocale();
  const english = locale === "en";
  const positionLabels = english ? ["Past", "Present", "Future"] : POSITIONS;
  ...
  const reversed = orientation === "reversed";
  const positionSentence = english
    ? enPositionParagraphOf(POSITION_KEYS[index], selected.slug)
    : koPositions[selected.slug]?.[POSITION_KEYS[index]];
  const themeParagraph = english
    ? enFocusParagraphOf(focus, selected.slug)
    : focusParagraphOf(focus, selected.slug, locale);
  const reversedPositionSentence = reversed
    ? english ? enPositionParagraphOf(POSITION_KEYS[index], selected.slug, true) : reversedPositions[selected.slug]?.[POSITION_KEYS[index]]
    : null;
  const reversedTheme = reversed
    ? english ? enFocusParagraphOf(focus, selected.slug, true) : reversedFocusParagraphOf(focus, selected.slug)
    : null;
  ...
  const uprightContent = (canonicalLabel: string) => (
    <>
      ...
      <CanonicalDetails label={canonicalLabel} paragraphs={descriptionOf(selected, locale)} />
    </>
  );
  ...
  return (
    ...
    >
      ...
      <p className="text-center text-[13px] text-muted lg:text-[14px]">
        {english ? "Past · Present · Future" : "과거 · 현재 · 미래"}
        <b className="mt-0.5 block font-medium text-gold">
          {focusLabelOf(focus, locale)}
        </b>
      </p>
      ...
      <div
        role="tablist"
        aria-label={english ? "Choose a position" : "포지션 선택"}
      ...
      >
        ...
        >
          ...
          <h2 className="mt-1 font-display text-[22px] font-semibold leading-tight lg:text-[26px]">
            {nameOf(selected, locale)}
            {reversed ? <ReversedBadge locale={locale} /> : null}
            {!english ? <span className="mt-0.5 block text-[13px] font-normal text-muted lg:text-[15px]">{selected.nameEn}</span> : null}
          </h2>
      ...
      <div className="mt-5 flex flex-col gap-2.5 lg:flex-row lg:flex-wrap lg:justify-center lg:gap-3.5">
        <ResultActions
          deckId={deckId}
          slug={selected.slug}
          readingId={readingId}
