"use client";

import Link from "next/link";
import { useState } from "react";
import { CircleNotch, UserCircle } from "@phosphor-icons/react";
import {
  isLocalAuthDev,
  setDevSession,
  signOutAndClear,
  useSession,
} from "@/lib/auth/session";
import { useSyncStatus, type SyncState } from "@/lib/sync/status";
import { useLocale } from "@/components/LocaleProvider";

const card =
  "rounded-2xl border border-line bg-ink-1 p-5 lg:rounded-[14px] lg:p-6";

/** 계정 카드 아래에 한 줄로 붙이는 동기화 상태 문구. */
function syncLine(state: SyncState, lastSyncedAt: string | null, english: boolean): string {
  if (english) {
    if (state === "syncing") return "Syncing…";
    if (state === "error") return "Waiting to sync · it will resume when connected";
    if (lastSyncedAt) return `Last synced ${new Date(lastSyncedAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}`;
    return "Your records are kept with your account";
  }
  if (state === "syncing") return "동기화 중…";
  if (state === "error") return "동기화 대기 중 · 연결되면 자동으로 올라갑니다";
  if (lastSyncedAt) {
    const d = new Date(lastSyncedAt);
    const hh = String(d.getHours()).padStart(2, "0");
    const mm = String(d.getMinutes()).padStart(2, "0");
    return `마지막 동기화 ${hh}:${mm}`;
  }
  return "기록이 계정에 보관됩니다";
}

function accountProviderLabel(provider: unknown, english: boolean): string {
  if (provider === "google") return english ? "Google account" : "Google 계정";
  if (provider === "kakao") return english ? "Kakao account" : "카카오 계정";
  if (provider === "development") return english ? "Local development account" : "로컬 개발 계정";
  return english ? "Social account" : "소셜 계정";
}

export function AccountCard() {
  const english = useLocale() === "en";
  const { user, loading, configured, devSession } = useSession();
  const { state, lastSyncedAt } = useSyncStatus();
  const [signingOut, setSigningOut] = useState(false);
  const [logoutError, setLogoutError] = useState<string | null>(null);

  if (loading) {
    return (
      <div className={`${card} min-h-[92px]`} role="status">
        <span className="sr-only">{english ? "Loading account information" : "계정 정보를 불러오는 중"}</span>
      </div>
    );
  }

  if (user) {
    const providerLabel = accountProviderLabel(user.app_metadata.provider, english);

    const handleSignOut = async () => {
      setSigningOut(true);
      setLogoutError(null);
      // 반환값은 "서버 revoke까지 성공했는가"다. 이 기기의 기록은 실패해도
      // 항상 정리되므로(S5), 실패 문구가 "정리되지 않았다"고 말하면 안 된다.
      const revoked = await signOutAndClear();
      if (!revoked) {
        setLogoutError(
          english ? "The sign-out request did not reach the server. Records on this device were cleared; check your connection and try again." : "로그아웃 요청이 서버에 닿지 않았습니다. 이 기기의 기록은 정리했습니다. 연결 상태를 확인하고 다시 시도해 주세요.",
        );
        setSigningOut(false);
      }
    };

    return (
      <div className={card}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <span className="flex min-w-0 items-center gap-3.5">
            <UserCircle size={22} className="text-gold-soft" aria-hidden />
            <span className="min-w-0">
              <span className="block font-display text-[16px] font-semibold lg:text-[18px]">
                {english ? "Account" : "계정 정보"}
              </span>
              <span className="block truncate text-[13px] text-body lg:text-[14px]">
                {user.email ?? (english ? "No email available" : "이메일 정보 없음")}
              </span>
              <span className="block text-[12px] text-muted lg:text-[13px]">
                {providerLabel}
              </span>
              <span className="block text-[12px] text-muted lg:text-[13px]">
                {devSession
                  ? (english ? "Local development mode · not syncing" : "로컬 개발 모드 · 동기화하지 않음")
                  : syncLine(state, lastSyncedAt, english)}
              </span>
            </span>
          </span>
          <button
            type="button"
            onClick={() => void handleSignOut()}
            disabled={signingOut}
            className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl border border-line px-4 text-[13px] text-muted transition-colors hover:border-line-gold hover:text-cream disabled:cursor-not-allowed disabled:opacity-60"
          >
            {signingOut ? (
              <CircleNotch size={16} className="animate-spin" aria-hidden />
            ) : null}
            {signingOut ? (english ? "Signing out…" : "로그아웃 중…") : (english ? "Sign out" : "로그아웃")}
          </button>
        </div>
        {logoutError ? (
          <p className="mt-3 text-[12.5px] text-gold-soft" role="alert">
            {logoutError}
          </p>
        ) : null}
      </div>
    );
  }

  return (
    <div className={card}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <span className="flex items-center gap-3.5">
          <UserCircle size={22} className="text-gold-soft" aria-hidden />
          <span>
            <span className="block font-display text-[17px] font-semibold lg:text-[19px]">
              {english ? "Connect an account" : "계정 연결"}
            </span>
            <span className="text-[13px] text-muted lg:text-[14px]">
              {configured
                ? (english ? "Sign in to keep your records safely across devices." : "로그인하고 기록을 기기 간에 안전하게 보관하세요.")
                : (english ? "You can check the sign-in configuration here." : "로그인 설정 상태를 확인할 수 있습니다.")}
            </span>
          </span>
        </span>
        <Link
          href="/login"
          className="btn btn-gold min-h-11 shrink-0 px-5 text-center"
        >
          {english ? "Go to sign in" : "로그인하러 가기"}
        </Link>
      </div>
      {isLocalAuthDev ? (
        <div className="mt-4 border-t border-dashed border-line pt-4">
          <p className="text-[12px] text-muted">
            로컬 개발 전용 · 실제 계정이나 동기화에는 연결되지 않습니다.
          </p>
          <button
            type="button"
            onClick={() => setDevSession(true)}
            disabled={devSession}
            className="mt-2 min-h-11 rounded-xl border border-line px-4 text-[13px] text-body transition-colors hover:border-line-gold hover:text-cream disabled:cursor-not-allowed disabled:opacity-50"
          >
            개발자 로그인으로 테스트
          </button>
        </div>
      ) : null}
    </div>
  );
}
