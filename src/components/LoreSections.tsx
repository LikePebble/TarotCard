...
export function LoreSections({ slug, deckId }: { slug: string; deckId: string }) {
  ...
  const lore = cardLore(slug, english ? "en" : "ko");
  if (!lore) return null;
    ...
    <section className="mt-7 lg:max-w-[560px]">
      {deckId !== "classic" && (
        <p className="mb-2 text-[12.5px] leading-relaxed text-muted">
          {english
            ? "These notes use the classic Rider-Waite-Smith tradition. Premium decks retell the same card in their own visual language, so their imagery may differ."
      ...
      <details className="border-t border-line">
        <summary className={SECTION_SUMMARY}>
          {english ? "Symbols in the card" : "카드 속 상징들"} <span aria-hidden className="text-muted">＋</span>
        </summary>
      ...
      <details className="border-t border-line">
        <summary className={SECTION_SUMMARY}>
          {english ? "Card story" : "카드 이야기"} <span aria-hidden className="text-muted">＋</span>
        </summary>
      ...
      <details className="border-y border-line">
        <summary className={SECTION_SUMMARY}>
          {english ? "Traditional correspondences" : "전통 대응"} <span aria-hidden className="text-muted">＋</span>
        </summary>
